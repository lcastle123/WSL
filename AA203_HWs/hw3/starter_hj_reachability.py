...
class PlanarQuadrotor:
    ...
    def optimal_control(self, state, grad_value):
        ...
        T1 = jnp.where(grad_value[1]*(jnp.cos(state[2])/self.m) - grad_value[3]*(self.l/self.Iyy) > 0, 0, self.max_thrust_per_prop)
        T2 = jnp.where(grad_value[1]*(jnp.cos(state[2])/self.m) + grad_value[3]*(self.l/self.Iyy) > 0, 0, self.max_thrust_per_prop)
...
def target_set(state):
    ...
    y, v_y, phi, omega = state
    lim1 = jnp.max(jnp.array([3-y, y-7]))
    lim2 = jnp.abs(v_y)-1
    lim3 = jnp.abs(phi)-jnp.pi/12
    lim4 = jnp.abs(omega)-1
    return jnp.max(jnp.array([lim1, lim2, lim3, lim4]))
    raise NotImplementedError
    #################################################################################
...
def envelope_set(state):
    ...
    y, v_y, phi, omega = state
    lim1 = jnp.max(jnp.array([1-y, y-9]))
    lim2 = jnp.abs(v_y)-6
    lim3 = jnp.abs(phi)-jnp.inf
    lim4 = jnp.abs(omega)-8
    return jnp.max(jnp.array([lim1, lim2, lim3, lim4]))
    raise NotImplementedError
    #################################################################################
...
fig.show()
fig.write_html("planar_quad_5.html")
