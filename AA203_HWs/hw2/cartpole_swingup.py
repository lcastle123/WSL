...
def ilqr(f, s0, s_goal, N, Q, R, QN, eps=1e-3, max_iters=1000):
    ...
    for _ in range(max_iters):
        ...
        p_vec = (s_bar[-1] - s_goal).T @ QN
        p_mat = QN
        # Backward pass:
        for k in reversed(range(N)):
            A_k = A[k]
            B_k = B[k]
            Q_xx = Q + A_k.T @ p_mat @ A_k
            Q_uu = R + B_k.T @ p_mat @ B_k
            Q_ux = B_k.T @ p_mat @ A_k
            Q_x = (s_bar[k] - s_goal).T @ Q + A_k.T @ p_vec
            Q_u = (u_bar[k]).T @ R + B_k.T @ p_vec

            y[k] = -np.linalg.inv(Q_uu) @ Q_u
            Y[k] = -np.linalg.inv(Q_uu) @ Q_ux

            p_vec = Q_x - Y[k].T @ Q_uu @ y[k]
            p_mat = Q_xx - Y[k].T @ Q_uu @ Y[k]
        ...
        for k in range(N):
            du[k] = y[k] + Y[k] @ ds[k]
            s_bar[k + 1] = f(s_bar[k], u_bar[k] + du[k])
            ds[k + 1] = s_bar[k + 1] - s_old[k + 1]
            u_bar[k] = u_old[k] + du[k]
        #######################################################################
