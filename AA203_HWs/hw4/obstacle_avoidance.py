...
def signed_distances(s, centers, radii):
    ...
    d = jnp.sqrt((s[0] - centers[:,0]) ** 2 + (s[1] - centers[:,1]) ** 2) - radii
...
def scp_iteration(f, d, s0, s_goal, s_prev, u_prev, P, Q, R):
    ...
    for k in range(N+1):
        for j in range(Ad.shape[1]):
            constraints.append(Ad[k][j] @ s_cvx[k] + cd[k][j] >= 0)
...
for t in tqdm(range(T)):
    ...
    s_mpc[t], u_mpc[t] = solve_obstacle_avoidance_scp(f, d, s, s_goal, N, P, Q, R,
        eps=eps, max_iters=N_scp, s_init=s_init, u_init=u_init, convergence_error=False)
