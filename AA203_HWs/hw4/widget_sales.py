...
for k in tqdm(range(1, num_epochs + 1)):
    ...
    for idx in shuffled_indices:
        s_t = log["s"][idx]
        s_idx = np.where(S == s_t)[0][0]
        a_t = log["a"][idx]
        a_idx = np.where(A == a_t)[0][0]
        r_t = log["r"][idx]
        s_next = log["s"][idx + 1] if idx + 1 < T else s_t
        s_next_idx = np.where(S == s_next)[0][0]

        # Update the value function
        Q[s_idx, a_idx] += (-α) * Q[s_idx, a_idx] + α * (r_t + γ * np.max(Q[s_next_idx]))
    # ############################# END PART (a) ##############################
    ...
    for s_idx, s in enumerate(S):
        for a_idx, a in enumerate(A):
            for d_idx, d in enumerate(D):
                ...
                s_next_idx = np.where(S == s_next)[0][0]
                r = reward(s, a, d)
                Q_vi[s_idx, a_idx] += P[d_idx] * (r + γ * (np.max(Q_vi_prev[s_next_idx]) if k > 0 else 0))
...
profit_ql = simulate(rng, lambda s: A[np.argmax(Q[int(s)])], T)[2].cumsum()

a_opt_vi = A[np.argmax(Q_vi, axis=1)]
profit_vi = simulate(rng, lambda s: A[np.argmax(Q_vi[int(s)])], T)[2].cumsum()
